...
def display_logits(tokenizer : PreTrainedTokenizerBase, tokens : List[int], logits : Int[Tensor, "batch position d_vocab"]):
    ...
    for pos in range(len(tokens)): 
        ...
        for rank, (prob, tid) in enumerate(zip(top_probs, top_ids), 1):
            records.append({
                "Position": pos,
